"""Real-data managed-run helpers for prepared datasets such as MNLogic."""
...
def execute_real_mnlogic_managed_run(
    ...
    *,
    ...
    """Execute one managed run on the prepared real MNLogic dataset."""

    random.seed(seed)
    torch.manual_seed(seed)

    benchmark_adapter = create_benchmark_adapter(benchmark_suite)
    if REAL_MNLOGIC_DATASET_NAME not in benchmark_adapter.list_datasets():
        raise ValueError(
            f"Benchmark suite '{benchmark_suite}' does not support dataset "
            f"'{REAL_MNLOGIC_DATASET_NAME}'."
        )

    runtime_context = build_mnlogic_runtime_context(
    ...
    evaluation_splits = build_real_evaluation_splits(
        ...
        ood_batches=(list(dataloaders.ood_loader) if dataloaders.ood_loader is not None else None),
    )

    resolved_run_name = run_name or f"real_mnlogic_{model_family}_seed_{seed}"
    config_snapshot = {
        ...
        "run": {
            ...
            "phase": 14,
            "seed": seed,
            "source": "r4_real_mnlogic_managed_run",
        },
        "selection": selection.to_dict(),
        "model": runtime_model_config,
        "dataset": {
            "name": REAL_MNLOGIC_DATASET_NAME,
...
def build_mnlogic_runtime_context(
    *,
    ...
    """Build dataset-aware runtime config and metadata for real MNLogic runs."""

    adapter = create_dataset_adapter(REAL_MNLOGIC_DATASET_NAME, dataset_root=dataset_root)
    ...
    if not logic_expression:
        raise ValueError(
            f"MNLogic runtime config requires metadata/source_info.json to define "
            f"'logic_expression': {source_info_path}"
        )

    positive_label_name, negative_label_name = _resolve_binary_label_names(label_definitions)
    positive_rule = _parse_logic_expression(
        logic_expression,
    ...
    if model_family == "pipeline":
        ...
        raise ValueError(f"Unsupported model family for real MNLogic runtime config: {model_family}")

    notes = dict(model_config.get("notes", {}))
    notes.update(
        {
            "runtime_dataset": REAL_MNLOGIC_DATASET_NAME,
            "runtime_logic_expression": logic_expression,
...
def _resolve_binary_label_names(
    ...
    if len(label_definitions) != 2:
        raise ValueError("Real MNLogic runtime config currently expects exactly two labels.")
    ...
    if positive_label_name is None or negative_label_name is None:
        raise ValueError(
            "Real MNLogic runtime config expects label IDs 0 and 1 to be present."
        )
    ...
    *,
    ...
    if isinstance(node, ast.Call) and isinstance(node.func, ast.Name):
        ...
        if operator_name == "not":
            ...
            return {"op": "not", "args": rendered_args}
        if not rendered_args:
            raise ValueError(f"Runtime logic operator '{node.func.id}' requires arguments.")
        return {"op": operator_name, "args": rendered_args}
