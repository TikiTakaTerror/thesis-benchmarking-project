#!/usr/bin/env python3
"""Train and validate the LTN model family on a synthetic dataset."""
...
from src.models.ltn_model import LTNModelAdapter
...
def main() -> int:
    ...
    model = LTNModelAdapter.from_config_dict(load_model_config("ltn"))
    print(
        f"[OK] Loaded LTN config with {model.config.num_concepts} concepts "
    ...
    print("[OK] Synthetic LTN dataset created: train=96, val=32")
    ...
    print(
        f"[OK] Train logic satisfaction: {train_metrics['train_logic_satisfaction']:.4f}"
    )
    ...
    reloaded_model = LTNModelAdapter.load_checkpoint(checkpoint_path)
    ...
    print("[OK] LTN smoke check passed.")
    return 0


def build_synthetic_dataset(
    model: LTNModelAdapter,
    *,
    total_samples: int,
